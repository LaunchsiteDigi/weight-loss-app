...
export default function LandingPage() {
  ...
  // If already authenticated (non-guest), redirect to /demo
  useEffect(() => {
    if (session?.user && !session.user.email?.startsWith("guest-")) {
      router.push("/demo");
    }
  }, [session, router]);
  ...
  useEffect(() => {
    if (state.status === "user_exists") {
      toast({ type: "error", description: "Account already exists! Try signing in." });
    ...
    <div className="flex min-h-dvh w-full flex-col items-center bg-[#f7fafc] dark:bg-background lg:flex-row">
      ...
      <div className="w-full px-6 pt-10 pb-6 lg:w-1/2 lg:px-12 lg:py-16">
        ...
        >
          ...
          <div className="mb-6">
            <p className="mb-4 text-lg font-bold text-foreground">What you get:</p>
          ...
          <div className="rounded-2xl border border-accent-foreground/20 bg-accent p-5">
            <p className="text-sm font-bold text-accent-foreground">About SlimZero</p>
      ...
      <div className="flex w-full items-center justify-center px-6 py-10 lg:w-1/2 lg:px-12 lg:py-16">
        ...
        >
          <div className="rounded-2xl border-2 border-border/50 bg-card p-8 shadow-lg">
            ...
            <p className="mb-8 text-sm text-muted-foreground">Start your weight loss journey today</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                />
                <p className="mt-1 text-[11px] text-muted-foreground">US number, no country code needed</p>
              </div>
